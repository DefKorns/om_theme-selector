...
The UI is my own standalone C++/SDL app (`theme_manager`), not a set of Lua scripts layered on the stock menu — it's built from source against the vendored engine (see [Building from source](#building-from-source)).
...
```sh
./build.sh compile   # cross-compiles theme_manager + theme_downloader via Docker
./build.sh hmod       # packages mod/ into out/*.hmod
```

`build.sh` pulls the toolchain and `vendor/OptionsMenu` submodules on first run and builds everything inside a Docker container — no local ARM toolchain needed.
